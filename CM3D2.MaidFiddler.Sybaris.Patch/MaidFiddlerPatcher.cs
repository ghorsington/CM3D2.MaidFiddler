...
namespace CM3D2.MaidFiddler.Sybaris.Patcher
{
    public static class MaidFiddlerPatcher
    {
        ...
        public static void Patch(AssemblyDefinition assembly)
        {
            ...
            maidType.GetMethod("SetThumIcon")
                    ?.InjectWith(thumbnailChangedHook, -1, 0, InjectFlags.PassInvokingInstance);
            ...
            for (PlayerChangeType e = PlayerChangeType.Days; e <= PlayerChangeType.ShopUseMoney; e++)
            {
                ...
                playerParam.GetMethod(addMethod)
                           .InjectWith(playerStatChangeHook,
                                       0,
                                       (int) e,
                                       InjectFlags.PassTag | InjectFlags.ModifyReturn);
