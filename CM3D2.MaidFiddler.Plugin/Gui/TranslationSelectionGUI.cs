    ...
    public partial class TranslationSelectionGUI : Form
    {
        ...
        private void OnApplyTranslation(object sender, EventArgs e)
        {
            TranslationData? translationData = listBox_translations.SelectedIndex != -1
                ? (TranslationData?) listBox_translations.SelectedItem
                : null;
            string langFileName;
            ...
            LoadingBarGUI loadingBarGui = new LoadingBarGUI(
                ...
                g =>
                {
                    HttpWebRequest webRequest =
                            (HttpWebRequest)
                            WebRequest.Create(
                                $"{MaidFiddler.RESOURCE_URL}/Resources/Translations/translation_list.txt");
                    g.Timer.Start();
                                ...
                                using (TextReader tr = new StringReader(sb.ToString()))
                                {
                                    if (!Translation.TagPattern.Match(tr.ReadLine()).Success)
                                    {
                                        Debugger.WriteLine(LogLevel.Error,
                                                           "Failed to parse the translation: No tag found!");
                                        MessageBox.Show(
