        ...
        public class MaidInfo
        {
            ...
            */
                ...
                Debugger.Assert(
                    () =>
                    {
                        foreach (KeyValuePair<MaidChangeType, Action<MaidChangeType>> maidParamsUpdater in
                            maidParamsUpdaters.Where(
                                maidParamsUpdater => maidParamsUpdater.Key <= MaidChangeType.Profile))
                        {
                            DataGridViewRow row;
            ...
            private void InitFunctions()
            {
                ...
                for (var e = MaidChangeType.Care; e <= MaidChangeType.TotalEvaluation; e++)
...
#pragma warning restore 642
            ...
            private void UpdateSexual()
            {
                Status maidStatus = Maid.Param.status_;
                object val = null;
                for (var e = MaidChangeType.SexualBack; e <= MaidChangeType.SexualThroat; e++)
                {
                    switch (e)
                    {
            ...
            private void UpdateCurrentNightWorkId()
            {
                gui.comboBox_work_night.SelectedIndex = Maid.Param.status.night_work_id == 0
                    ? -1
                    : gui.nightWorkIDToRow[Maid.Param.status.night_work_id];
            }
            ...
            private void UpdateCurrentNoonWorkId()
            {
                gui.comboBox_work_noon.SelectedIndex = Maid.Param.status.noon_work_id == 0
                    ? -1
                    : gui.noonWorkIDToRow[Maid.Param.status.noon_work_id];
            }
                ...
                Debugger.Assert(
                    () =>
                    {
                        ...
                        if (UpdateFunctions.TryGetValue(type, out updateVal))
                            ...
                            Debugger.WriteLine(LogLevel.Error,
                                               $"No update function found for {EnumHelper.GetName(type)}!");
                        gui.valueUpdate[type] = false;
                    },
                    $"Failed to update maid field for type {EnumHelper.GetName(type)}");
            }
            ...
            {
                UpdateYotogiClass(
                    (int) Maid.Param.status.GetType()
                              .GetProperty("current_yotogi_class")
                              .GetValue(Maid.Param.status, null),
                    -1);
            }
            ...
            private void UpdateCurrentMaidClass()
            {
                UpdateMaidClass(
                    (int) Maid.Param.status.GetType()
                              .GetProperty("current_maid_class")
                              .GetValue(Maid.Param.status, null),
                    -1);
            }
