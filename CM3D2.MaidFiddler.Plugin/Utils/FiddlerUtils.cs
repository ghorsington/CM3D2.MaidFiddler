            ...
            try
            {
                if (GameVersion < MaidFiddler.SUPPORTED_MIN_CM3D2_VERSION)
                {
                    ...
                    text = Translation.IsTranslated("ERROR_OLD_GAME_VERSION")
                           ? string.Format(Translation.GetTranslation("ERROR_OLD_GAME_VERSION"), MaidFiddler.VERSION, $"{GameVersion / 100}.{GameVersion % 100}", $"{MaidFiddler.SUPPORTED_MIN_CM3D2_VERSION / 100}.{MaidFiddler.SUPPORTED_MIN_CM3D2_VERSION % 100}")
